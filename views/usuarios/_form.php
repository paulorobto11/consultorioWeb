...
Modal::begin([
    'id' => 'mbFoto',
    'header'=>'<h4 class="modal-title text-primary"><i class="fa fa-camera"></i> Foto do Usuario</h4>',    
]);
    ...
    echo FileInput::widget([
		...
		'pluginOptions' => [
			...
			'uploadLabel' =>  'Enviar',
			'uploadUrl' => Url::to(['/usuarios/enviar-foto']),
		],
    ]);
    ActiveForm::end();
Modal::end();
...
$(document).ready(function(){
    ...
    $('#btnRemoverFoto').click(function(){
        $.confirm({
            ...
            confirm: function(){
                $('#img-usuario').attr('src', URL_USER_IMG);
    ...
    $('#usuario-imagem').on('fileuploaded', function(event, data, previewId, index) {
        var retorno = data.response;
        try {
            if (retorno.foto_caminho != undefined && retorno.foto_nome != undefined){
                $('#img-usuario').attr('src', '/' + retorno.foto_caminho);
                $('#usuario-foto').val(retorno.foto_nome);
				$.ajax({
					type     :'GET',
					cache    : false,
					url  : '/usuario/atualiza-imagem?img=' + retorno.foto_nome,
					success  : function(response) {
				    	 if (response != 404){
							var resp = $.parseJSON(response);
							console.log(resp);
