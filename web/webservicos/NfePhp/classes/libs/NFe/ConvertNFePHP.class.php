...
class ConvertNFePHP
{
     ...
     */
    public function nfetxt2xml($txt,$qrcode,$ambiente)
    {
        if (is_file($txt)) {
        ...
        return $this->nfeTxt2XmlArrayComLinhas($aDados,$qrcode,$ambiente);
    } //fim nfetxt2xml
    ...
    protected function nfeTxt2XmlArrayComLinhas($arrayComAsLinhasDoArquivo,$qrcode,$ambiente)
    {
            ...
            switch ($dados[0]) {
                ...
                case "A":
                    ...
                    unset($dom, $NFe, $infNFe, $ide, $cUF, $cNF, $natOp,
                          ...
                          $prod, $cProd, $cEAN, $xProd, $NCM, $NVE, $CEST,  $EXTIPI, $CFOP,
                ...
                case "E02":
                	if ($this->modnfe == 65) {
	                    if ($this->tpAmb == '2') {
	                        ...
	                        if (substr($dados[1],0,9) != '00000000000') {
//	       	                    $infNFe->appendChild($dest);
		                        $CNPJ = $dom->createElement("CNPJ", $dados[1]);
		                        $this->docto = $dados[1];
	                            $dest->insertBefore($dest->appendChild($CNPJ), $xNome);
                ...
                case "E03":
                	if ($this->modnfe == 65) {
	                    if ($this->tpAmb == '2') {
	                        ...
	                        if (substr($dados[1],0,9) != '00000000000') {
//	       	                    $infNFe->appendChild($dest);
		                        $CNPJ = $dom->createElement("CPF", $dados[1]);
		                        $this->docto = $dados[1];
	                            $dest->insertBefore($dest->appendChild($CNPJ), $xNome);
	                            $infNFe->appendChild($dest);
	                        }
                ...
                case "I":
                    //PRODUTO SERVICO [det]
                    //I|cProd|cEAN|xProd|NCM|CEST|EXTIPI|CFOP|uCom|qCom|vUnCom|vProd|cEANTrib|uTrib|qTrib|vUnTrib
                    ...
                    $CEST = $dom->createElement("CEST", $dados[6]);
                ...
                case "I05c":
                    //Tag da Codificacao NVE
                    //I05a|NVE|
                    $CEST = $dom->createElement("CEST", $dados[1]);
                    $prod->appendChild($CEST);
                    break;
                case "I18":
                ...
                case "YA":
                    ...
                    if(!empty($dados[3])){
                      ...
                      $Integra = $dom->createElement("tpIntegra",$dados[4]);
                      $card->appendChild($Integra);
                      $CNPJ = $dom->createElement("CNPJ",$dados[5]);
                      $card->appendChild($CNPJ);
                      $tBand = $dom->createElement("tBand",$dados[6]);
                      $card->appendChild($tBand);
                      $cAut = $dom->createElement("cAut",$dados[7]);
        ...
        foreach ($notas as $nota) {
            ...
            if (!empty($infNFe)) {
                ...
                // Inclus�o da linha do QRCODE para NFCE - mod = 65 //
                $modelo  = $ide->getElementsByTagName('mod')->item(0)->nodeValue;
                if ($modelo == 65) {
                	if ($this->docto) {
	                ...
	                $urlQR = $this->urlConsulta[$this->tpAmb][12]['QR'];
		            $dados = $infNFe->C14N(false, false, null, null);
		            $hashValue = hash('sha1', $dados, true);
		            $digVal = base64_encode($hashValue);
	                $idToken = '';
	                $Token = '';
	                ...
	                $infQRCODE = $dom->createElement("qrCode");
	                $infQRCODE->appendChild($dom->createCDATASection($this->imgQRCode)); 
	                $infNFeSupl->appendChild($infQRCODE);
	                $NFe->appendChild($infNFeSupl);
                }
...
//			$sAmb = 'homologacao';
//			$dir = PATH_ARQS.'/'.$sAmb.'/entradas/';
			if (!file_put_contents($dir.$this->chave.'.xml', $arquivos_xml)) {
				return false;
			}
		}
        return $this->chave;
