...
require_once PATH_ROOT.'webservicos/NfePhp/classes/libs/Common/DomDocumentNFePHP.class.php';
require_once PATH_ROOT.'webservicos/NfePhp/classes/libs/Common/ExceptionNFePHP.class.php';
...
class ToolsNFePHP extends CommonNFePHP
{
    ...
    public $raizDir = PATH_DIR;
    public $arqDir = '';
    public $arqsDir = PATH_ARQS;
    public $pdfDir = '';
    public $entDir = '';
    public $valDir = '';
    public $repDir = '';
    ...
    public function __construct($aConfig = '', $mododebug = 2, $exceptions = false)
    {
        ...
        $this->entDir=$this->arqsDir.$sAmb.DIRECTORY_SEPARATOR.'entradas'.DIRECTORY_SEPARATOR;
        $this->assDir=$this->arqsDir.$sAmb.'/'.'assinadas'.'/';
        $this->valDir=$this->arqsDir.$sAmb.DIRECTORY_SEPARATOR.'validadas'.DIRECTORY_SEPARATOR;
        $this->rejDir=$this->arqsDir.$sAmb.DIRECTORY_SEPARATOR.'rejeitadas'.DIRECTORY_SEPARATOR;
        $this->envDir=$this->arqsDir.$sAmb.DIRECTORY_SEPARATOR.'enviadas'.DIRECTORY_SEPARATOR;
        ...
        $this->canDir=$this->arqsDir.$sAmb.DIRECTORY_SEPARATOR.'canceladas'.DIRECTORY_SEPARATOR;
        $this->inuDir=$this->arqsDir.$sAmb.DIRECTORY_SEPARATOR.'inutilizadas'.DIRECTORY_SEPARATOR;
        $this->cccDir=$this->arqsDir.$sAmb.DIRECTORY_SEPARATOR.'cartacorrecao'.DIRECTORY_SEPARATOR;
        $this->evtDir=$this->arqsDir.$sAmb.DIRECTORY_SEPARATOR.'eventos'.DIRECTORY_SEPARATOR;
        $this->dpcDir=$this->arqsDir.$sAmb.DIRECTORY_SEPARATOR.'dpec'.DIRECTORY_SEPARATOR;
        $this->temDir=$this->arqsDir.$sAmb.DIRECTORY_SEPARATOR.'temporarias'.DIRECTORY_SEPARATOR;
        $this->recDir=$this->arqsDir.$sAmb.DIRECTORY_SEPARATOR.'recebidas'.DIRECTORY_SEPARATOR;
        $this->conDir=$this->arqsDir.$sAmb.DIRECTORY_SEPARATOR.'consultadas'.DIRECTORY_SEPARATOR;
        $this->pdfDir=$this->arqsDir.$sAmb.DIRECTORY_SEPARATOR.'pdf'.DIRECTORY_SEPARATOR;
        
        if (!is_dir($this->arqDir)) {
            mkdir($this->arqDir, 0777);
        }
        if (!is_dir($this->arqDir.'/'.$sAmb)) {
            mkdir($this->arqDir.'/'.$sAmb, 0777);
        }
    ...
    public function signXML($docxml, $tagid = '')
    {
        try {
            if ($tagid == '') {
     echo "<hr>ddd ".           $msg = "Uma tag deve ser indicada para que seja assinada!!";
                throw new nfephpException($msg);
            }
            if ($docxml == '') {
     echo "<hr>ddd ".           $msg = "Um xml deve ser passado para que seja assinado!!";
                throw new nfephpException($msg);
            }
            if (! is_file($this->priKEY)) {
     echo "<hr>ddd ".           $msg = "Arquivo \"$this->priKEY\" da chave privada parece invalido, verifique!!";
                throw new nfephpException($msg);
            }
            if (is_file($docxml)) {
...
//            	            	            	            	            echo "<hr>sss ".$retorno;
            ...
            if ($indSinc == 1) {
                ...
                $nome = $this->aprDir.$infProt['chNFe'].'-prot.xml';
            } else {
                ...
                $nome = $this->aprDir.$idLote.'-rec.xml';
            }
...
//                print_r ($retorno);
     ...
     *
     * ESSE SEVIÇO NÃO EST�? TOTALMENTE OPERACIONAL EXISTE APENAS NO SEFAZ DO RS E SVAN
     *
     * Este serviço não suporta SVC !!
     *
     * @name getNFe
    ...
    public function envia_nfe($xml, &$aRetorno = array())
    {
    	$indSinc = 0;
    	$idLote  = 1;
    	...
    	. "<enviNFe xmlns=\"$this->URLPortal\" versao=\"$versao\">"
    	. "<idLote>$idLote</idLote>"
    	. "<indSinc>$indSinc</indSinc>$xml</enviNFe></nfeDadosMsg>";

    	$param = array('arg0' => $cabec,'arg1' => $dados);
