<?php 
 /** Classe responsavel pelas transacoes da Tabela: Destinatario. */ 
class DestinatarioDAO { 
	  ...
	  function insert($obj){ 
		 ...
		 $sql .= '\''.$obj->getNome_destinatario().'\',';
		 $sql .= ($obj->getTipo()?$obj->getTipo():0).',';
		 $sql .= '\''.$obj->getCpf().'\',';
		 $sql .= '\''.$obj->getCnpj().'\',';
		 $sql .= '\''.$obj->getRg().'\',';
		 $sql .= ($obj->getInscmun()?$obj->getInscmun():0).',';
	  ...
	  function update($obj){
		 ...
		 $sql .= 'nome_destinatario = '.'\''.$obj->getNome_destinatario().'\' ,';
		 $sql .= 'tipo = '.($obj->getTipo()?$obj->getTipo():0).',';
		 $sql .= 'cpf = '.'\''.$obj->getCpf().'\' ,';
		 $sql .= 'cnpj = '.'\''.$obj->getCnpj().'\' ,';
		 $sql .= 'rg = '.'\''.$obj->getRg().'\' ,';
		 $sql .= 'inscmun = '.($obj->getInscmun()?$obj->getInscmun():0).',';
	  ...
	  function delete($obj){ 
		 $sql = 'DELETE FROM destinatario WHERE codemp = '.$obj->getCodemp();
		$results = mysql_query($sql);
		if (!$results) {
			return false;
		} else {
	  ...
	  function select($obj){ 
		 $sql = 'SELECT * FROM destinatario WHERE 1=1 '; 
		if ($obj->getCodemp()) { 
		...
		if ($obj->getNome_destinatario()) { 
			$sql .= " AND upper(nome_destinatario)  like ('%".strtoupper($obj->getNome_destinatario())."%')";
		} 
		if ($obj->getTipo()) { 
		...
		if ($obj->getInscmun()) { 
			$sql .= " AND inscmun = '".$obj->getInscmun()."'";
		} 
		if ($obj->getSexo()) { 
		...
		if ($obj->getFoto_cliente()) { 
		 ...
		 if(mysql_num_rows($rs) > 0 ){ 
			 ...
			 $obj->setNome_destinatario(mysql_result($rs, 0, 'nome_destinatario')); 
			 ...
			 $obj->setInscmun(mysql_result($rs, 0, 'inscmun')); 
	  ...
	  function countAll($obj){
		 $sql = 'SELECT count(*) as existe FROM destinatario WHERE 1=1 ';  
 	  ...
 	  function selectAll($obj){
		...
		if ($obj->getFoto_cliente()) { 
		 ...
		 for($i = 0; $i < mysql_num_rows($rs); $i++){ 
			 ...
			 $obj->setNome_destinatario(mysql_result($rs, $i, 'nome_destinatario')); 
			 ...
			 $obj->setInscmun(mysql_result($rs, $i, 'inscmun')); 
