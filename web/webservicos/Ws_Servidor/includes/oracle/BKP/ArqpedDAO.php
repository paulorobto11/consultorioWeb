...
class ArqpedDAO { 
	  ...
	  function conta_venda($codemp,$data){
	 		$sql = "SELECT count(*) as qtde FROM arqped where codemp = ".$codemp." and data_venda = '".$data."'";
	
			$query = mysql_query($sql);
			$dados = mysql_fetch_assoc($query);
			$idvenda 	= $dados['qtde']+1;
			return $idvenda;
	  }
		
	  
 	  function contador_controle($codemp,$data){
			$sql = "SELECT max(id_controle) as controle FROM arqped where codemp = ".$codemp." and data_venda = '".$data."'";;
			$query = mysql_query($sql);
			$dados = mysql_fetch_assoc($query);
			$idcontrole = $dados['controle']+1;
			return $idcontrole;
	  }
