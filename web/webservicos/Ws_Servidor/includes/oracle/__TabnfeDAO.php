...
class TabnfeDAO { 
	  ...
	  function select($obj){ 
		...
		if ($obj->getVol_aVol()) { 
			$sql .= " AND upper(vol_aVol)  like ('%".strtoupper($obj->getVol_aVol())."%')";
		} 
		...
		if ($obj->getTotal_vtotal()) { 
		 ...
		 if(mysql_num_rows($rs) > 0 ){ 
			 ...
			 $obj->setVol_aVol(mysql_result($rs, 0, 'vol_aVol')); 
 	  ...
 	  function selectAll($obj){
		...
		if ($obj->getTotal_vtotal()) { 
		 ...
		 for($i = 0; $i < mysql_num_rows($rs); $i++){ 
			 ...
			 $obj->setVol_aVol(mysql_result($rs, $i, 'vol_aVol')); 
