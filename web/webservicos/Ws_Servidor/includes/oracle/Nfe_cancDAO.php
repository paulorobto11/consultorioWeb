<?php 
 /** Classe responsavel pelas transacoes da Tabela: Nfe_canc. */ 
class Nfe_cancDAO { 
 
	  function insert($obj){ 
	 	 $sql = 'INSERT INTO nfe_canc ('; 
		 $sql .= 'seq_nfe,';
		 $sql .= 'codemp,';
		 $sql .= 'data_envio,';
		 $sql .= 'anomes,';
	  ...
	  function delete($obj){ 
		 $sql = 'DELETE FROM nfe_canc WHERE seq_nfe = '.$obj->getSeq_nfe();
		$results = mysql_query($sql);
		if (!$results) {
			return false;
		} else {
	  ...
	  function select($obj){ 
		 $sql = 'SELECT * FROM nfe_canc WHERE 1=1 '; 
		if ($obj->getSeq_nfe()) { 
	  ...
	  function countAll($obj){
		 $sql = 'SELECT count(*) as existe FROM nfe_canc WHERE 1=1 ';  
	  ...
	  function proximo(){
		 $sql = 'SELECT max(seq_nfe) as existe FROM nfe_canc WHERE 1=1 ';  
		 $query = mysql_query($sql); 
		 $dados = mysql_fetch_assoc($query);  
		 $existe = $dados['existe']; 
		 return $existe; 
